...
1. For function calls:
   FUNCTION_CALL: [REASONING_TYPE] function_name|param1|param2|...

2. For final Mathematical answers:
   FINAL_ANSWER: [final_function_response]

Important:
- Prefix each function call with the reasoning type used: [ARITHMETIC], [GEOMETRIC], [TRIGONOMETRIC], [LOOKUP], [STRING], etc.
- Always perform a sanity check on intermediate results before using them in further calculations. If the result appears incorrect, retry or revise prior steps.
- When a function returns multiple values, process all of them.
- Do not repeat function calls with the same parameters.
- If a tool fails or gives unexpected results, respond with:
  FUNCTION_CALL: error_handling|function_name|error_description
- If you're unsure how to proceed, respond with:
  FUNCTION_CALL: unsure|reason
...
- Once you reach the end of the mathematical steps, proceed to do the following MS paint tasks:
    ...
    4. ALWAYS Ensure the text box is STRICTLY within the rectangle boundary drawn by you

Only give FINAL_ANSWER when you have completed all necessary calculations and steps.

Examples:
- FUNCTION_CALL: [ARITHMETIC] add|5|3
- FUNCTION_CALL: [STRING] strings_to_chars_to_int|INDIA
- FUNCTION_CALL: [UTILITY] open_paint
- FUNCTION_CALL: [GEOMETRIC] draw_rectangle|450|700|780|500
- FUNCTION_CALL: [UTILITY] add_text_in_paint|Hello world|500|600
- FUNCTION_CALL: error_handling|sqrt|result returned None
- FUNCTION_CALL: unsure|not sure if input should be int or list
"""
                ...
                while iteration < max_iterations:
                    ...
                    if not response_text.startswith("FINAL_ANSWER:"):
                        ...
                        ##find reasoning type
                        match = re.search(r'\[([^\[\]]+)\]', function_info)
                        if match:
                            reasoning_type = match.group(0)  # includes the square brackets
                            if reasoning_type  is not None:
                                function_info = function_info.replace(reasoning_type, '').strip()

                        parts = [p.strip() for p in function_info.split("|")]
                        func_name, params = parts[0], parts[1:]
                        
                        logging.info(f"\nRaw function info: {function_info}")
                        logging.info(f"Reasoning type: {reasoning_type}")
                        ...
                        try:
                            ...
                            if not tool:
                                if func_name == "error_handling":
                                    logging.info(f"Error handling tool called")
                                    raise Exception(function_info)
                                elif func_name == "unsure":
                                    logging.info(f"Unsure tool called")
                                    raise Exception(function_info)
                                else:
