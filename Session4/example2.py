...
@mcp.tool()
async def add_text_in_paint(text: str, x_text: int, y_text: int) -> dict:
    """Add text in Paint"""
    global paint_app
    try:
        if not paint_app:
        ...
        paint_window.click_input(coords=(346, 96))
        time.sleep(0.5)
        ...
        canvas.click_input(coords=(x_text,y_text))
        time.sleep(0.5)
...
async def open_paint() -> dict:
    ...
    try:
        ...
        win32gui.SetWindowPos(
            paint_window.handle,
            win32con.HWND_TOP,
            0, 0,  # Position it on secondary monitor primary_width + 1
...
def get_greeting(name: str) -> str:
    ...
    logging.info("CALLED: get_greeting(name: str) -> str:")
    return f"Hello, {name}!"
...
def review_code(code: str) -> str:
    ...
    logging.info("CALLED: review_code(code: str) -> str:")
